  ...
  <svg class="line-chart" viewBox="0 0 700 240" preserveAspectRatio="none">
    <defs>
      <linearGradient id="scoreGrad" x1="0" x2="1">
        ...
        <stop offset="1" stop-color="#0f4ca3" />
      </linearGradient>
    </defs>
    <line v-for="n in 5" :key="n" x1="44" x2="680" :y1="gridY(n)" :y2="gridY(n)" stroke="#e4ebf5" stroke-width="1" />
    <polyline v-if="points.length" :points="points" fill="none" stroke="url(#scoreGrad)" stroke-width="4" stroke-linejoin="round" stroke-linecap="round" />
    <circle v-for="p in pointList" :key="p.label" :cx="p.x" :cy="p.y" r="4" fill="#092b6f" />
...
<style scoped>
.line-chart { width:100%; height:260px; background:white; border-radius:18px; }
</style>
