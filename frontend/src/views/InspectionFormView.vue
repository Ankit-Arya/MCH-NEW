<template>
  <AppLayout>
    <section class="hero-panel card entry-hero">
      <div>
        ...
        <p class="hero-subtitle">
          Select only the area being inspected. Each saved entry belongs to the
          same inspection number, with its own attribute, sub-area, grade,
          evidence and capture metadata.
        </p>
      </div>
    ...
    <div v-else-if="loadError" class="card section-gap load-error-card">
      ...
      <p>{{ loadError }}</p>
      <p class="muted">
        If this started after access-control mapping, confirm that the logged-in
        user has station access for this inspection. Admin can map it from
        Access Control.
      </p>
      ...
      <div class="card capture-card">
        <div class="card-title">
          <div>
            ...
            <p class="muted">
              Photo evidence is mandatory as per selected sub-area master data.
              Save Entry is unlocked only after the required photos are
              captured/selected.
            </p>
          </div>
        </div>
    ...
    <div v-if="!loading && !loadError" class="card submit-panel section-gap">
      <div>
        ...
        <p class="muted">
          You can submit with partial selected entries. Each saved entry must
          satisfy the mandatory photo count from master data.
        </p>
      </div>
      <div class="submit-actions">
        <button class="btn btn-muted" @click="loadEntries">
          Refresh Entries
        </button>
        <button
          class="btn btn-primary"
          @click="submitInspection"
          :disabled="submitting || !entries.length || !canEdit"
        >
...
const canEdit = computed(() =>
  ["DRAFT", "RETURNED_FOR_CLARIFICATION"].includes(inspection.value?.status),
);
const statusClass = computed(() =>
  inspection.value?.status === "DRAFT" ? "amber" : "green",
);
const selectedPhotoFiles = computed(() => getPhotoFiles());
const requiredPhotoCount = computed(() =>
  Math.max(1, Number(selectedSubArea.value?.photo_min_required || 1)),
);
const maxPhotoCount = computed(() =>
  Math.max(
    requiredPhotoCount.value,
    Number(selectedSubArea.value?.photo_max_allowed || 3),
  ),
);
const canSaveCurrentEntry = computed(
  () =>
    canEdit.value &&
    !!selectedSubArea.value &&
    selectedPhotoFiles.value.length >= requiredPhotoCount.value &&
    !saving.value,
);
...
function captureGps() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      gpsError.value = "GPS permission denied or unavailable";
      metadata.captured_at = nowIso();
    },
...
async function loadSubAreas(attributeId) {
  ...
  try {
    subAreas.value = (
      await api.get(`/master/inspection-attributes/${attributeId}/sub-areas`)
    ).data;
  } finally {
...
async function loadEntries() {
  entries.value = (
    await api.get(`/inspections/${route.params.id}/entries`)
  ).data;
}
...
async function uploadFile(entryId, file, mediaType) {
  ...
  fd.append(
    "file",
    file,
    file.name || `${mediaType.toLowerCase()}-${Date.now()}`,
  );

  // Let the browser/axios set the multipart boundary automatically.
  await api.post(
    `/inspections/${route.params.id}/entries/${entryId}/media`,
    fd,
  );
}
...
async function saveEntry(form) {
  ...
  message.value = "";
  if (!canEdit.value) {
    error.value = "This inspection is already submitted/locked";
    return;
  }
  ...
  if (photos.length < minRequired) {
    error.value = `This sub-area requires ${minRequired} photo${minRequired === 1 ? "" : "s"}. Selected: ${photos.length}`;
    return;
  }
  if (photos.length > maxAllowed) {
    error.value = `Maximum ${maxAllowed} photo${maxAllowed === 1 ? "" : "s"} allowed for this sub-area`;
    return;
  }
  ...
  try {
    ...
    const { data: entry } = await api.post(
      `/inspections/${route.params.id}/entries`,
      payload,
    );
...
async function submitInspection() {
  ...
  try {
    const { data } = await api.post(`/inspections/${route.params.id}/submit`, {
      remarks: inspection.value?.remarks || null,
    });
    inspection.value = data;
    message.value = "Inspection submitted for review.";
  } catch (e) {
    error.value = apiErrorText(e, "Unable to submit inspection");
  } finally {
...
onMounted(async () => {
  ...
  try {
    ...
    checklist.value = {
      ...
      grades: check.grades || check.grading_options || [],
    };
    await loadEntries();
    captureGps();
  } catch (e) {
    loadError.value =
      ...
      "Unable to load inspection form. Please check API logs and station access mapping.";
  } finally {
    loading.value = false;
  }
});
