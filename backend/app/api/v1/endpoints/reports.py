...
def _inspection_score(inspection: Inspection) -> float:
    if not inspection.attribute_scores:
        # Entry-based inspections may not have attribute_scores in older reports.
        # Keep this as 0.0 instead of failing PDF/report screens.
        return 0.0
    return round(sum(s.grade_percentage for s in inspection.attribute_scores) / len(inspection.attribute_scores), 2)


def _paginate_query(query, page: int, size: int):
    """Return a stable pagination envelope used by Vue pages.

    Frontend lists must not fetch hundreds/thousands of rows at once.
    This helper keeps the API response predictable:
    {
      items: [...], total: 142, page: 1, size: 20, pages: 8,
