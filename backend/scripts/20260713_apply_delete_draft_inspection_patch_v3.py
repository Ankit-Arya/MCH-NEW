...
def _find_project_paths() -> Tuple[Path, Path]:
    """Return (project_root, backend_dir) no matter where this script is run from.

    The V1 script assumed backend/scripts/.. incorrectly on Windows. This version
    searches upward for backend/app/api/v1/endpoints/inspections.py so it works
    when copied into backend/scripts and executed from project root.
    """
    ...
    for candidate in candidates:
        ...
        if (backend / "app" / "api" / "v1" / "endpoints" / "inspections.py").exists():
            return candidate, backend

        if (candidate / "app" / "api" / "v1" / "endpoints" / "inspections.py").exists():
            return candidate.parent, candidate

    raise RuntimeError(
        "Could not find backend/app/api/v1/endpoints/inspections.py. "
        "Run this script from the mch-inspection-platform project root or copy it into backend/scripts."
    )


PROJECT, BACKEND = _find_project_paths()
...
def main() -> None:
    ...
    print(f'Delete draft inspection patch applied successfully. Backend={BACKEND} Frontend={PROJECT / "frontend"}')
